...
for r1, r2 in pairwise(corners + [corners[0]]):
    ...
    red_lines.append((r1, r2))
corners = set(corners)

def range_inside(x, y):
    return range(min(x, y) + 1, max(x, y))

part1 = part2 = 0
n_combs = len(corners) * (len(corners) - 1) // 2 - 1
for i, (t1, t2) in enumerate(combinations(corners, 2)):
    print(f"{i}/{n_combs}")
    xs = sorted([t1[0], t2[0]])
    ys = sorted([t1[1], t2[1]])
    # tiles = set(product(range(xs[0], xs[1] + 1), range(ys[0], ys[1] + 1)))
    ...
    tr = (t2[0], t1[1])
    bl = (t1[0], t2[1])
    borders = [(t1, tr), (tr, t2), (t2, bl), (bl, t1)]
    # for rl1, rl2 in red_lines:
